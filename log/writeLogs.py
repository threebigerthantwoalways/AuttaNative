...
class MyLogs:
    ...
    def __init__(self, log_path):
        ...
        runtime = time.strftime('%Y-%m-%d', time.localtime(time.time()))#'2022-10-19'
        logfile = os.path.join(log_path, runtime+'.log')
        logfile_err = os.path.join(log_path, runtime+'_error.log')#拼接log日志的路径
        ...
        fh = logging.FileHandler(logfile, mode='a+') #a+ append模式 递增不覆盖
