...
from traffic.mitm_addon import saveToSQLite
from config import *

async def start_mitmproxy_async(system):
    ...
    mitm.addons.add(saveToSQLite(system))
    try:
        await mitm.run()
...
# 只启动抓包功能
# redisPort: 启动redis端口
def start_traffic(system):
    asyncio.run(start_mitmproxy_async(system))
